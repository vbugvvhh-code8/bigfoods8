...
export default function ResetPasswordPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setDone(true);
      setTimeout(() => router.push('/order/login'), 1800);
    } catch (e: any) {
  ...
  return (
    <div className="w-full max-w-[380px] mx-auto px-4 py-14">
      ...
      {done ? (
        <p className="text-[13px]" style={{color: 'var(--green, #1F8A5C)'}}>
        ...
        <>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <button
              ...
              className="w-full py-3.5 rounded-xl text-[13px] font-semibold text-white disabled:opacity-40 flex items-center justify-center gap-2 mt-1"
              style={{background: 'var(--orange)'}}
            >
          ...
          <p className="text-[12px] text-center mt-4" style={{color: 'var(--gray)'}}>
            {resent ? (
              'New code sent — check your spam folder if it doesn\'t arrive shortly.'
            ) : (
              <>
