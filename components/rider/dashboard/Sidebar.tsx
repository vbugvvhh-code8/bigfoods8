...
function LogoutButton() {
  ...
  async function handleLogout() {
    ...
    router.push('/rider-portal/login');
  }
...
export default function Sidebar({ riderName }: { riderName?: string }) {
  ...
  return (
    <>
      ...
      <div className="hidden md:flex w-[200px] flex-shrink-0 flex-col py-5 px-3.5" style={{ background: 'var(--ink)', color: '#D8D2CB' }}>
        <div className="mb-7">
          <BrandBlock />
          <p className="text-[10px] uppercase tracking-widest px-1.5 mt-2" style={{ color: '#9C948A' }}>Rider Portal</p>
        </div>
        <NavList />
        {footer}
      </div>
